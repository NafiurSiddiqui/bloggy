...
@if (isset($noHref) && $noHref)
    <div
        class="inline-block bg-[#DEEDE8] dark:bg-darkLabel dark:text-darkText-100 text-center text-emerald-700 font-semibold {{ $sm ? 'text-xs px-2' : 'text-sm px-4' }}  py-1 min-w-4">
        {{ $slot }}
    </div>
@else
    <a href="{{ $url }}"
