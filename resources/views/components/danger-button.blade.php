@props(['submit', 'label', 'button-only', 'form'])

@if (isset($submit) || isset($buttonOnly))
    <button type="{{ isset($submit) ? 'submit' : 'button' }}"
        {{ $attributes->merge(['form' => isset($form) ? $form : 'form-delete', 'class' => 'border border-rose-200 dark:text-rose-500 font-medium hover:text-rose-600  ms-3 py-2 px-4 rounded text-rose-400']) }}>{{ $label ?? 'Delete' }}</button>
@else
    <span
